...
public final class ActionBarDisplay {
     ...
     */
    @NonNull
    public static Builder builder(@NonNull Action action) {
     ...
     * <p>Example:</p>
     *
     * <pre>[동작 이름] <font color="yellow">■■■■</font><font color="black">■■■■■■</font> [(아이콘) 40.5]  [1][좌클릭] 해제</pre>
     *
     * <pre><code>
     * ActionBarDisplay actionBarDisplay = ActionBarDisplay.builder(action)
     *     .title()
     *     .durationBar()
     *     .keyInfo("해제")
     *     .build();
    ...
    public static final class Builder {
        ...
        /**
         * 제목을 {@link Action#getDisplayName()}으로 설정한다.
         *
         * <p>Example:</p>
         *
         * <pre>[스킬 이름]</pre>
        ...
        public Builder title() {
            return title(action.getDisplayName());
        }
         ...
         *
         * @return {@link Builder}
         * @throws IllegalArgumentException 동작이 {@link ChargeableSkill}을 상속받지 않으면 발생
         */
        @NonNull
        public Builder progressBar() {
            Validate.isTrue(action instanceof ChargeableSkill, "action이 ChargeableSkill을 상속받지 않음");
            return progressBar((int) ((ChargeableSkill) action).getStateValue(), (int) ((ChargeableSkill) action).getMaxStateValue(), 10,
         ...
         *
         * @return {@link Builder}
         * @throws IllegalArgumentException 동작이 {@link Skill}을 상속받지 않으면 발생
         */
        @NonNull
        public Builder durationBar() {
            Validate.isTrue(action instanceof Skill, "action이 Skill을 상속받지 않음");
            return durationBar(((Skill) action).getDuration(), ((Skill) action).getDefaultDuration());
        }
        ...
        public Builder cooldownBar() {
            return cooldownBar(action.getCooldown(), action.getDefaultCooldown());
        }
         ...
         * @param symbol 막대 기호
         * @return {@link Builder}
         * @throws IllegalArgumentException 인자값이 유효하지 않거나 동작이 {@link Reloadable}을 상속받지 않으면 발생
         */
        @NonNull
        public Builder ammoBar(int length, char symbol) {
            Validate.isTrue(length >= 1, "length >= 1 (%d)", length);
            Validate.isTrue(action instanceof Reloadable, "action이 Reloadable을 상속받지 않음");

            ReloadModule reloadModule = ((Reloadable) action).getReloadModule();
            progressBar(reloadModule.getRemainingAmmo(), reloadModule.getCapacity(), length, symbol);

            String prefix = TextIcon.CAPACITY.toString();

            Weapon weapon = action.getCombatUser().getActionManager().getWeapon();
            if (weapon instanceof Swappable && (action == weapon) != ((Swappable<?>) weapon).getSwapModule().isSwapped())
                prefix = "§a" + prefix;
        ...
        public Builder keyInfo(@NonNull String description) {
            return keyInfo(description, action.getDefaultActionKeys().toArray(new ActionKey[0]));
        }
