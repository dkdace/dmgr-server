...
public final class DummyProjectile extends Projectile<Damageable> {
    ...
    /** 아군 총알 궤적 입자 효과 */
    private static final ParticleEffect TEAM_BULLET_TRAIL_PARTICLE =
            ParticleEffect.Colored.builder(ParticleEffect.Colored.ParticleType.REDSTONE, Color.fromRGB(60, 255, 60)).count(3)
                    .horizontalSpread(0.05).verticalSpread(0.05).build();
    /** 발사 효과음 */
    private static final SoundEffect SHOOT_SOUND =
            SoundEffect.builder(Sound.ENTITY_SHULKER_SHOOT).volume(0.8).pitch(1.3).build();
