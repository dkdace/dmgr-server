...
/**
 * 다수의 소환 가능한 엔티티({@link SummonEntity})를 소환할 수 있는 스킬의 모듈 클래스.
 *
 * @param <T> {@link SummonEntity}를 상속받는 소환 가능한 엔티티
 */
public final class MultiSummonModule<T extends SummonEntity<?>> {
    /** 최대 엔티티 수 */
    ...
    /**
     * 다중 엔티티 소환 모듈 인스턴스를 생성한다.
     *
     * @param skill    대상 스킬
     * @param maxCount 최대 엔티티 수. 1 이상의 값
     * @throws IllegalArgumentException 인자값이 유효하지 않으면 발생
     */
    public MultiSummonModule(@NonNull Skill skill, int maxCount) {
     ...
     * 소환한 엔티티를 추가한다.
     *
     * <p>소환한 엔티티 수가 {@link MultiSummonModule#maxCount} 이상이면 가장 오래된 엔티티를 제거한 뒤 추가한다.</p>
     *
